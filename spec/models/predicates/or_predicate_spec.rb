...
describe OrPredicate, type: :model do

  it 'has a valid factory' do
    expect(FactoryGirl.build(:or_predicate)).to be_valid
  end
  ...
  describe '#is_true?' do
    let(:predicate) { FactoryGirl.create(:or_predicate, predicate1: predicate1, predicate2: predicate2) }
    ...
    context 'one predicate is true' do
      it 'should return true' do
        expect(predicate.is_true?(nil, { cool: 'value' })).to eq(true)
        expect(predicate.is_true?(nil, { awesome: 'value' })).to eq(true)
      end
    end
