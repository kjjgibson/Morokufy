  ...
  describe '#send_points_awarded_notification' do
    ...
    it 'should send the request' do
      expect_any_instance_of(HipChat::HipChatRequest).to receive(:send_room_notification) do |_, room, room_notification|
        ...
        expect(card.description).to eq('<b>Bob</b> has been awarded <b>10</b> points for a successful Semaphore build.')
        ...
        expect(attributes[1].label).to eq('Achievements')
        expect(attributes[1].value.label).to eq(3)
      end

      gs_player = GameServer::Model::Player.new('', '', '', '')
      gs_player.points = 100
      gs_player.achievements = [0, 0, 0] # We only care about the size of this array, not what's in it - so fill it with anything
      MorokufyHipChatNotifications.new().send_points_awarded_notification(10, 'Bob', gs_player, GameServer::Admin::Request::ExternalEventRequest::EventTypes::SEMAPHORE_BUILD_PASSED_EVENT)
    end
  end
